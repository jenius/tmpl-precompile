...
(function() {
  var Namespacer, Precompiler, async, colors, extend, extractFunction, fs, globalSettings, jade, jsp, optimizeOutput, path, pro, util, _ref;
  ...
  */
  ...
  Precompiler = (function() {
    ...
    */
    ...
    Precompiler.prototype.getNamespaces = function(cb) {
      var _this = this;
      return Namespacer(this.settings, function(err, res) {
        if (err != null) {
          throw err;
        } else {
          return cb(res.join('\n') + '\n');
    ...
    */
    ...
    Precompiler.prototype.compile = function() {
      var _this = this;
      return this.getNamespaces(function(namespaces) {
        var buf, helpers, inline, output, template, templates, uglify, _i, _len, _ref1, _ref2;
        _ref1 = _this.settings, templates = _ref1.templates, inline = _ref1.inline, helpers = _ref1.helpers, uglify = _ref1.uglify, output = _ref1.output;
        buf = [];
        buf.push("(function(){");
        if (namespaces !== false) {
          buf.push(namespaces);
        ...
        if (helpers !== false && inline !== true) {
          buf.push(_this.helpers());
        }
        _ref2 = _this.settings.templates;
        for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
          template = _ref2[_i];
          buf.push(_this.compileTemplate(template).toString());
        }
        buf.push("})();");
        buf = buf.join("");
        if (uglify !== false) {
          buf = _this.uglifyOutput(buf);
        }
        if (output != null) {
          fs.writeFileSync(_this.settings.output, buf);
          if (_this.settings.verbose) {
            console.log(('Saving ' + (uglify !== false ? 'and Uglifying ' : '')).bold + ':' + output);
          }
        }
        if (_this.callback != null) {
          return _this.callback(null, buf);
        }
      });
    };
